...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(total, small): # start from the smallest
        # base case
        if total == 0:
            return 1
        elif total < small:
            return 0
        # include small + not include small
        return helper(total - small, small) + helper(total, small*2)
...
def make_anonymous_factorial():
    ...
    # 目标：lambda x: 1 if x == 1 else mul(x, fact(x, 1))
    # 这道题比较抽象...
