...
def balanced(m):
    ...
    len_left, len_right = length(left_arm), length(right_arm)
    w_left, w_right = total_weight(end(left_arm)), total_weight(end(right_arm))
    return (len_left * w_left == len_right * w_right) and (balanced(end(left_arm)) and balanced(end(right_arm)))
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def rid_list(l, new_l=[]):
        for item in l:
            if type(item) == type(l):
                new_l + rid_list(item,new_l)
            else:
                new_l.append(item)
        return new_l
    if is_leaf(t):
        ...
        l = [label(t)]
        l_app = [preorder(branch) for branch in branches(t)]
        return rid_list(l + l_app, [])
    # 用了一个非常魔怔的方法
    # 其实直接列表展开就行了hh
