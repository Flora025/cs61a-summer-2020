...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pwd):
        nonlocal balance, incorrect_pwd, locked
        if locked:
            return f"Too many incorrect attempts. Attempts: {incorrect_pwd}"
        if pwd == password:
            if balance < amount:
                return 'Insufficient funds'
            balance -= amount
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(c) == str:
        ...
        def joint_withdraw(amount, any_pass):
            nonlocal old_pass, new_pass
            return withdraw(amount, old_pass if any_pass == new_pass else any_pass)
