...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        "*** YOUR CODE HERE ***"
        # eval operator
        operator = self.operator.eval(env) # Name('add')
        # eval operands
        operands = [operand.eval(env) for operand in self.operands] # [Literal(3), Name('a')]
        # eval expression
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        d_tmp = self.parent.copy()
        for name, value in zip(self.parameters, arguments):
            d_tmp[name] = value
        return self.body.eval(d_tmp)
