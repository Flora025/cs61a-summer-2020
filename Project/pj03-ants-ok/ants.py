    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self, ContainerAnt):
                if ContainerAnt.can_contain(self, place.ant):
                    self.contain_ant(place.ant)
                    place.ant = self
            elif isinstance(place.ant, ContainerAnt):
                if ContainerAnt.can_contain(place.ant, self):
                    place.ant.contain_ant(self)
            else:
                assert place.ant is None, 'Two ants in {0}'.format(place)  # 暂时不允许两个container
            # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        while check_place and (check_place is not beehive):
            if_in_range = (count >= self.min_range) and (count <= self.max_range if self.max_range is not None else True)
            if len(check_place.bees) != 0 and if_in_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        if_dying = 1 if (self.armor-amount) <=0 else 0
        bees_here = self.place.bees[:]
        for bee in bees_here:
            bee.reduce_armor(amount + if_dying * self.damage)
...
class HungryAnt(Ant):
    ...
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
    time_to_digest = 3  # by default
    # END Problem 6

    def __init__(self, armor=1):   # 这里。。。是继承ant的armor而不是自行赋值
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        self.digesting = 0
        Ant.__init__(self, armor)
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.digesting == 0:
            if len(self.place.bees) != 0:             # not digesting and has >0 bee
                lucky_bee = rANTdom_else_none(self.place.bees)
                self.eat_bee(lucky_bee)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate): # ok-case 7是不是有问题？
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        bees_here = self.place.bees #list
        if len(bees_here) != 0:   # has bee(s)
            for bee in bees_here:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.impostor:
            ...
            # throw a leaf
            super().action(gamestate)
            # doubles damage if behind queen
            check_position = self.place.exit # Place instance
            while check_position is not None:
                if check_position.ant is not None: #there's ant(s) in the PLACE
                    if not isinstance(check_position.ant, ContainerAnt): # there is one and it is not container ant
                        if not check_position.ant.have_buffed:
                            check_position.ant.damage *= 2
                            check_position.ant.have_buffed = True
                    elif isinstance(check_position.ant, ContainerAnt):       # one ant but container ant  可以简化一半 但是懒得写了
                        if check_position.ant.contained_ant is None:
                            check_position.ant.damage *= 2
                            check_position.ant.have_buffed = True 
                        else: # two ants and one of them container                          
                            if not check_position.ant.contained_ant.have_buffed:
                                check_position.ant.contained_ant.damage *= 2
                                check_position.ant.contained_ant.have_buffed = True
                            if not check_position.ant.have_buffed:
                                check_position.ant.damage *= 2
                                check_position.ant.have_buffed = True
                # if there isn't any ant, check the next place
                check_position = check_position.exit
        # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant:
            if self.place.ant.blocks_path is True:
                return True
            else:
                return False
        else:
